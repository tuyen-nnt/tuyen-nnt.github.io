...
for index, d in df.iterrows():
    clinton += word_in_text('clinton', d['text'])
    trump += word_in_text('trump', d['text'])
    sanders += word_in_text('sanders', d['text'])
    cruz += word_in_text('cruz', d['text'])
```

* Tiếp theo ta sẽ tạo list trong Python, mỗi items sẽ có giá trị đếm bắt đầu =0. Mục đích ở đây là để đếm số tweet gặp phải cho mỗi keyword.
